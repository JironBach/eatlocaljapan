...
#{EasyTranslate.translate(@listing.visit_benefits_another, to: :en)}" : @listing.visit_benefits_another_en
  ...
  def search
    ...
    categories_id.push(ActiveRecord::Base.send(:sanitize_sql_array, ["select listing_id from listings_shop_categories where shop_category_id in (:ids)", ids: categories_id])) unless categories_id.blank?
